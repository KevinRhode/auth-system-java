    ...
    <div class="company-page">
      @if (loading()) {
        ...
        <div class="empty-state">
          ...
          <a routerLink="/company/create" class="btn btn-primary"> Create Company </a>
        </div>
      } @else {
...
export class CompanyComponent implements OnInit {
  ...
  invite() {
    ...
    this.companyService.inviteMember(this.company()!.id, email, role).subscribe({
      next: (member) => {
        this.company.update((c) => (c ? { ...c, members: [...(c.members || []), member] } : c));
    ...
    this.companyService.updateMemberRole(this.company()!.id, memberId, role).subscribe({
      next: (updated) => {
        this.company.update((c) =>
          c ? { ...c, members: c.members.map((m) => (m.id === memberId ? updated : m)) } : c,
        );
      },
    });
  }
  ...
  removeMember(memberId: string) {
    ...
    this.companyService.removeMember(this.company()!.id, memberId).subscribe({
      next: () => {
        this.company.update((c) =>
          c ? { ...c, members: c.members.filter((m) => m.id !== memberId) } : c,
        );
      },
    });
  }
